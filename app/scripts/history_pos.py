def start_hisotry_pos():
    ...
    def print_first_row():
        ...
        mycursor.execute(f"SELECT from_who FROM {login_user+pos} LIMIT 1")
        myresult = mycursor.fetchall()

        for x_who in myresult:
            who = str(x_who[0])
            #print(who)
    ...
    def print_other_rows():
        ...
        while True:
            ...
            mycursor.execute(f"SELECT from_who FROM {login_user+pos} LIMIT 1 OFFSET {offset}")
            myresult = mycursor.fetchall()
